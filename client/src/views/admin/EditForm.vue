    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">商品情報編集</h5>
        ...
        <div class="modal-body">
          <form>
            ...
            <div class="form-group mb-3" :class="{ invalid: descriptionError }">
              ...
              <input type="text" class="form-control" v-model.lazy="description" @blur="handleDescription" />
            ...
            <div class="form-group mb-3" :class="{ invalid: categoryError }">
              ...
              <select class="custom-select mr-sm-2 category" v-model.lazy="category" @blur="handleCategory">
        ...
        <div class="modal-footer">
          <button type="submit" @click="handleUpdateProduct" data-bs-dismiss="modal" class="btn btn-primary">
            更新
          </button>
          <button type="button" class="btn btn-danger" data-bs-dismiss="modal">取り消し</button>
        </div>
      </div>
    </div>
  </div>
...
export default {
  name: "EditForm",
  props: {
    findedProductProp: Object,
    loadFindedProductProp: Boolean
  },
  setup(props, { emit }) {
    ...
    const { value: description, errorMessage: descriptionError, handleChange: handleDescription } = useField(
      "description"
    );
    const { value: category, errorMessage: categoryError, handleChange: handleCategory } = useField("category");
    watchEffect(() => {
      props.loadFindedProductProp;
      name.value = props.findedProductProp.name;
      image.value = props.findedProductProp.image;
      price.value = props.findedProductProp.price;
      description.value = props.findedProductProp.description;
      category.value = props.findedProductProp.category;
    });
    const handleUpdateProduct = handleSubmit(updatedProduct => {
      emit("handle-update-product", updatedProduct);
    });
